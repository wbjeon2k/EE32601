...
//------------------------Parameter----------------------
`define TV_IN_a "../tv/cdatafile/c.gcd_custom.autotvin_a.dat"
`define TV_IN_b "../tv/cdatafile/c.gcd_custom.autotvin_b.dat"
`define TV_OUT_gcd "../tv/rtldatafile/rtl.gcd_custom.autotvout_gcd.dat"
...
parameter gcd_DEPTH = 1;
reg [31 : 0] gcd_OPERATE_DEPTH = 0;
parameter gcd_c_bitwidth = 32;
parameter a_data_in_addr = 16;
parameter b_data_in_addr = 24;
parameter gcd_data_out_addr = 32;
parameter gcd_valid_out_addr = 36;
...
reg [DATA_WIDTH - 1 : 0] mem_gcd [gcd_DEPTH - 1 : 0];
reg gcd_read_data_finish;
...
//read gcd reg
reg [31 : 0] read_gcd_count = 0;
reg read_gcd_run_flag = 0;
reg read_one_gcd_data_done = 0;
...
assign TRAN_control_read_data_finish = 1 & gcd_read_data_finish;
assign TRAN_control_write_data_finish = 1 & a_write_data_finish & b_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        gcd_read_data_finish <= 0;
        read_gcd_run_flag <= 0; 
        read_gcd_count = 0;
        count_operate_depth_by_bitwidth_and_depth (gcd_c_bitwidth, gcd_DEPTH, gcd_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            read_gcd_run_flag = 1; 
        ...
        if (read_one_gcd_data_done === 1) begin
            read_gcd_count = read_gcd_count + 1;
            if (read_gcd_count == gcd_OPERATE_DEPTH) begin
                read_gcd_run_flag <= 0; 
                gcd_read_data_finish <= 1;
            end
        end
    end
end

initial begin : read_gcd
    integer read_gcd_resp;
    integer process_num;
    integer get_vld;
    integer four_byte_num;
    integer c_bitwidth;
    ...
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = gcd_c_bitwidth;
    process_num = 2;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_2_finish <= 0;
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            if (read_gcd_run_flag === 1) begin
                process_busy = 1;
                get_vld = 0;
                //read gcd vld
                read (gcd_valid_out_addr, RDATA_reg, read_gcd_resp);
                ...
                if (get_vld == 1) begin
                    //read gcd data 
                    for (i = 0 ; i < four_byte_num ; i = i+1) begin
                        read (gcd_data_out_addr + read_gcd_count * four_byte_num * 4 + i * 4, RDATA_reg, read_gcd_resp);
                        if (gcd_c_bitwidth < 32) begin
                            mem_gcd[read_gcd_count] <= RDATA_reg;
                        end
                        else begin
                            for (j=0 ; j < 32 ; j = j + 1) begin
                                if (i*32 + j < gcd_c_bitwidth) begin
                                    mem_gcd[read_gcd_count][i*32 + j] <= RDATA_reg[j];
                                end
                            end
                        end
                    end
                    
                    read_one_gcd_data_done <= 1;
                    @(posedge clk);
                    read_one_gcd_data_done <= 0;
                end    
                process_busy = 0;
            end    
            process_2_finish <= 1;
...
initial begin : write_gcd_file_proc 
  integer fp; 
  integer factor; 
  integer transaction_idx; 
  reg [gcd_c_bitwidth - 1 : 0] mem_tmp; 
  reg [ 100*8 : 1] str;
  integer i; 
  transaction_idx = 0; 
  count_seperate_factor_by_bitwidth (gcd_c_bitwidth , factor);
  while(1) begin 
      @(posedge clk);
      while (gcd_read_data_finish !== 1 || TRAN_control_transaction_done_in === 1) begin
          @(posedge clk);
      end
      # 0.1;
      fp = $fopen(`TV_OUT_gcd, "a"); 
      if(fp == 0) begin       // Failed to open file 
          $display("Failed to open file \"%s\"!", `TV_OUT_gcd); 
          $finish; 
      end 
      $fdisplay(fp, "[[transaction]] %d", transaction_idx);
      for (i = 0; i < (gcd_DEPTH - gcd_DEPTH % factor); i = i + 1) begin
          if (factor == 4) begin
              if (i%factor == 0) begin
                  mem_tmp = mem_gcd[i/factor][7:0];
              end
              if (i%factor == 1) begin
                  mem_tmp = mem_gcd[i/factor][15:8];
              end
              if (i%factor == 2) begin
                  mem_tmp = mem_gcd[i/factor][23:16];
              end
              if (i%factor == 3) begin
                  mem_tmp = mem_gcd[i/factor][31:24];
              end
              $fdisplay(fp,"0x%x",mem_tmp);
          end
          if (factor == 2) begin
              if (i%factor == 0) begin
                  mem_tmp = mem_gcd[i/factor][15:0];
              end
              if (i%factor == 1) begin
                  mem_tmp = mem_gcd[i/factor][31:16];
              end
              $fdisplay(fp,"0x%x",mem_tmp);
          end
          if (factor == 1) begin
              $fdisplay(fp,"0x%x",mem_gcd[i]);
          end
      end 
      if (factor == 4) begin
          if ((gcd_DEPTH - 1) % factor == 2) begin
              $fdisplay(fp,"0x%x",mem_gcd[gcd_DEPTH / factor][7:0]);
              $fdisplay(fp,"0x%x",mem_gcd[gcd_DEPTH / factor][15:8]);
              $fdisplay(fp,"0x%x",mem_gcd[gcd_DEPTH / factor][23:16]);
          end
          if ((gcd_DEPTH - 1) % factor == 1) begin
              $fdisplay(fp,"0x%x",mem_gcd[gcd_DEPTH / factor][7:0]);
              $fdisplay(fp,"0x%x",mem_gcd[gcd_DEPTH / factor][15:8]);
          end
          if ((gcd_DEPTH - 1) % factor == 0) begin
              $fdisplay(fp,"0x%x",mem_gcd[gcd_DEPTH / factor][7:0]);
          end
      end
      if (factor == 2) begin
          if ((gcd_DEPTH - 1) % factor == 0) begin
              $fdisplay(fp,"0x%x",mem_gcd[gcd_DEPTH / factor][15:0]);
          end
      end
