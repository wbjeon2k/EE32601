...
typedef struct {
    ...
    XGcd_custom_uio_map maps[ MAX_UIO_MAPS ];
} XGcd_custom_uio_info;

/***************** Variable Definitions **************************************/
static XGcd_custom_uio_info uio_info;
...
static int uio_info_read_name(XGcd_custom_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XGcd_custom_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XGcd_custom_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XGcd_custom_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XGcd_custom_Initialize(XGcd_custom *InstancePtr, const char* InstanceName) {
	XGcd_custom_uio_info *InfoPtr = &uio_info;
