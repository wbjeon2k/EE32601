...
entity gcd_custom_control_s_axi is
generic (
...
end entity gcd_custom_control_s_axi;
...
architecture behave of gcd_custom_control_s_axi is
    ...
    constant ADDR_GCD_DATA_0 : INTEGER := 16#20#;
    constant ADDR_GCD_CTRL   : INTEGER := 16#24#;
    ...
    signal int_gcd             : UNSIGNED(31 downto 0) := (others => '0');
    signal int_gcd_ap_vld      : STD_LOGIC;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_GCD_DATA_0 =>
                        rdata_data <= RESIZE(int_gcd(31 downto 0), 32);
                    when ADDR_GCD_CTRL =>
                        rdata_data(0) <= int_gcd_ap_vld;
                    when others =>
                        NULL;
                    end case;
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (gcd_ap_vld = '1') then
                    int_gcd <= UNSIGNED(gcd); -- clear on read
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (gcd_ap_vld = '1') then
                    int_gcd_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_GCD_CTRL) then
                    int_gcd_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
