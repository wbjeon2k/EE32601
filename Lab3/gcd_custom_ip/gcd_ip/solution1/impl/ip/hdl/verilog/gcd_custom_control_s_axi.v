...
// ==============================================================
`timescale 1ns/1ps
module gcd_custom_control_s_axi
#(parameter
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_GCD_DATA_0: begin
                    rdata <= int_gcd[31:0];
                end
                ADDR_GCD_CTRL: begin
                    rdata[0] <= int_gcd_ap_vld;
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (gcd_ap_vld)
            int_gcd_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_GCD_CTRL)
            int_gcd_ap_vld <= 1'b0; // clear on read
    end
end
